...
void Betweener::readCVInputs(void){
    ...
    if (msecTickerCVRead >= analog_read_delta){
        ...
        currentCV1 = analogRead(ANALOG1) / 8;
        currentCV2 = analogRead(ANALOG2) / 8;
        currentCV3 = analogRead(ANALOG3) / 8;
        currentCV4 = analogRead(ANALOG4) / 8;
    }
}
...
void Betweener::readKnobs(void){
    ...
    currentKnob1 = analogRead(KNOB1) / 8;
    currentKnob2 = analogRead(KNOB2) / 8;
    currentKnob3 = analogRead(KNOB3) / 8;
    currentKnob4 = analogRead(KNOB4) / 8;
}
